...
HDF5convertor::HDF5convertor(QObject *parent, QString datasetpath, QString outputdir,QString type, QString filetype)
	: QObject(parent)
...
bool HDF5convertor::convertFile()
{
	...
	cv::Point textOrg((_outputSize.width - textSize.width) / 2, margin - (margin - textSize.height) / 2 );
	...
	if (_type == "video")
	{
		...
		for (hsize_t frameIdx = 0; frameIdx < _dims[0]; frameIdx++)
		{
			...
			_bufferFrame.copyTo(_outputFrame(cv::Rect(0, margin, _bufferFrame.cols, _bufferFrame.rows)));
