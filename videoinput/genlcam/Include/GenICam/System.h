...
public:
	...
	/// \~chinese
	/// \brief 发现指定接口类型可达的设备
	/// \param [out] _CameraPtrList 指定接口类型所有在线设备对象列表,当interfaceType = typeAll 时，发现所有接口下的在线设备
	/// \param [in]  interfaceType  接口类型
	/// \return 是否成功
	bool discovery(Infra::TVector<ICameraPtr>& _CameraPtrList, EInterfaceType interfaceType = typeAll);
