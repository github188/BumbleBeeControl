...
public:
	...
	/// 通过线程_bubbleBeeID获取线程名称
	/// \param [in] _bubbleBeeID 线程_bubbleBeeID
	/// \name [out] 线程名称
	/// \return true 成功
	/// \return false 失败
	bool getThreadNameByID(int32_t _bubbleBeeID, CString& name);

	/// 得到超时的线程名称
	bool getTimeOutThreads(CString2& names);

	///\brief 线程队列中是否已存在此线程
	///\param [in] _bubbleBeeID 需要检测的线程_bubbleBeeID
	///\return TRUE表示此线程在队列中，否则表示不存在
	bool hasThread(int _bubbleBeeID);
